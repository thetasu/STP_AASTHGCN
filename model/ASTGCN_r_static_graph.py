...
class AAST_block(nn.Module):
    ...
    def __init__(self, DEVICE, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, num_of_vertices, num_of_timesteps, cheb_polynomials):
...
class ASTGCN_framework_classification(nn.Module):
    ...
    def __init__(self, DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, num_for_predict, len_input, num_of_vertices,cheb):
        '''
        :param nb_block:
        :param in_channels:
        :param K:
        ...
        self.BlockList = nn.ModuleList([AAST_block(DEVICE, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, num_of_vertices, len_input,cheb)])

        self.BlockList.extend([AAST_block(DEVICE, nb_time_filter, K, nb_chev_filter, nb_time_filter, 1, num_of_vertices, len_input // time_strides, cheb) for _ in range(nb_block - 1)])
...
def make_model(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, num_for_predict, len_input, num_of_vertices,adj):
    '''
    ...
    model = ASTGCN_framework_classification(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, num_for_predict, len_input, num_of_vertices,cheb_polynomials)
